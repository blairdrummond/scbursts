...
This package is designed to extract information on the stochastic properties of single molecules. It was originally designed for dwell time analysis of single channel data derived from patch clamp experiments, though it might have uses outside of the domain that it was designed for. It contains functions for importing and exporting idealized stochastic events; displaying, analyzing and correcting dwells; defining and sorting bursts, or clusters of bursts. One of the main aims of the package is to create a single platform for unifying and comparing data produced by different programs and stored in different file formats; for now that means that `scbursts` can read and write the following:

Program | File extension | URL
 -- | -- | --
TAC | `evt` | [https://www.bruxton.com/TAC/index.html](https://www.bruxton.com/TAC/index.html)
MIL | `dwt` | [https://qub.mandelics.com/](ihttps://qub.mandelics.com/)
DCPROGS | `scan`  | [https://github.com/DCPROGS](:https://github.com/DCPROGS)
Clampfit | `xls` | [https://www.moleculardevices.com/products/axon-patch-clamp-system/acquisition-and-analysis-software/pclamp-software-suite](https://www.moleculardevices.com/products/axon-patch-clamp-system/acquisition-and-analysis-software/pclamp-software-suite)

**Double check QUB v.s. MIL**

but `scbursts` is an open-source project designed to be extensible for other labs and their needs -- support for new file formats is easily added, and since the project is written in `R`, new functionality may also be introduced over time. Those interested in requesting or (even better) adding new functionality to the project are invited to create an issue or pull request on [the github page](https://github.com/dacostalab/scbursts) (if you are familiar with the tool), or [contact our lab](https://www.dacosta.net/).

**This manual gives a high-level overview on using the package. If you want to use/learn about the package, please cite/read the official white-paper. For "low-level" details of specific functions, use `help()` or consult the technical manual (the source code is also available to read)**

\tableofcontents

# `.evt`s, `.dwt`s, and `.scntxt`s

EVTs (Event files) and DWTs (Dwell times) are formats produced and required by several pieces of software that are used by the lab. In particular, TAC produces and uses `.evt` files and MIL uses `.dwt`s. So we need to read and write these files often, but they aren't ideal ways to represent our data, and so the only manipulations we want to do on `evt`s and `dwt`s is read and write them to data-types which are more natural in R. Those data-types which we prefer are **segments**, and **bursts**, which are detailed in the next sections. Segments are essentially tables of dwell times, and bursts are essentially lists of segments, but with meta-data attached to them. In any case **the first thing you want to do is convert your file into a list of segments**.

## Handling `.evt`s

In the case of TAC `evt` files, when reading in, files transition through three states:

$$ \tt{evt\ file} \to \tt{Table\ of\ transition\ times} \to \tt{Table\ of\ dwells} $$

In order to read a TAC `evt` file, type something like:
...
To write an `evt` you simply type:
...
`.dwt` files record sequences of dwells, which alternate between open and closed states (at least, they should. Though sometimes there are errors. This will be discussed later). There can be multiple "segments" of dwells, each corresponding to a continuous stretch of idealized events.
...
**segment** is also the name of the data-type `scbursts` defines, so that reading a `dwt` gives you a list of `segment`s (which is an `R` object, like a `data.frame`). Often one has that each segment is meant to correspond to a burst, and so when we say "bursts", typicically we refer to a list of segments. As a consequence, there are also functions in `scbursts` such as `bursts.select` which make working with these lists more convenient.

*The reason `dwt`s yield a list of segments is because there are often several continuous stretches of recording seperated by pauses.*
...
# Transition times and states (1=Open, 0=Closed, units are in seconds)
head(dwells[[1]])
```
...
In the Ion-Channel analysis community, **Bursts** are usually defined by a critical closed duration (\(t_{crit}\)), which is determined from analysis of a closed dwell duration histogram. This \(t_{crit}\) stipulates that openings separated by closings briefer than \(t_{crit}\) originate from the same burst of single-channel activity, while openings separated by closings longer than \(t_{crit}\) originate from different bursts (ref.).


In scbursts, the scientific notion of **bursts** and the program definition of `bursts` *usually* coincide, but not always. As mentioned, a `segment` denotes a contiguous sequence of idealized dwells, however idealized single channel recordings can be made up of one **or more** segments. Furthermore, each segment can contain multiple **bursts** (in the scientific sense). So the unfortunate side effect of this is that `scbursts` reads in data files like `evt`s and `dwt`s as `bursts`, but as a list of recording intervals `[recordings]`, not as actual **bursts**, which would be `[burst]` (The square brackets denote a list). However this is easily resolved; one just has to pass this list through a burst detector. So, for example, a function like `bursts.defined_by_tcrit` will break up the recordings into lists of bursts (using an input critical time). The example workflow below shows what this would look like:
...
# Turn the transition times into dwells
# 'records' is technically a 'bursts' object, but
# it isn't actually a true list of bursts (yet)
...
# to turn the records inti actual bursts
...
Since **bursts** are attributable to the activity of a single channel, **bursts** are the units that we are interested in for kinetic analysis of single channels.
...
# dwells <- scan.read('example1_scan.txt')
...
infile <- system.file("extdata", "example_qub.dwt", package = "scbursts")
dwells <- dwt.read(infile)
...
# dwells <- clampfit.read('Figure1.xlsx')
